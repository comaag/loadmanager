...
function () {
  ...
  _createClass(loadManager, [{
    ...
    value: function getScripts() {
      ...
      var withOnRequestScripts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    ...
    value: function load() {
      ...
      this.setLevel(level);
      if (level == 0) return;
      var scripts = this.getScripts(level, false);
      scripts.forEach(function (script) {
      ...
      var checkComplete = function checkComplete(script) {
        ...
        if (counter >= scripts.filter(function (script) {
          return !script.onRequest;
        }).length) {
